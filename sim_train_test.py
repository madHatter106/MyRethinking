...
pl.scatter(np.arange(1, 6).astype('int'), train, color='b', label='train')
pl.scatter(np.arange(1, 6).astype(int), test, color='k', label='test')
pl.legend()
pl.xticks(np.arange(1, 6))
pl.ylabel('deviance')
pl.xlabel('# parameters');

test_waic = [dm['m%d' %i]['waic_test'].WAIC for i in range(1, 6)]
train_waic = [dm['m%d' %i]['waic_train'].WAIC for i in range(1, 6)]
pl.scatter(np.arange(1, 6).astype('int'), train_waic, color='b', label='train')
pl.scatter(np.arange(1, 6).astype('int'), test_waic, color='k', label='test')
pl.legend()
pl.xticks(np.arange(1, 6))
pl.ylabel('WAIC')
pl.xlabel('# parameters');
test_loo = [dm['m%d' %i]['loo_test'].LOO for i in range(1, 6)]
train_loo = [dm['m%d' %i]['loo_train'].LOO for i in range(1, 6)]
pl.scatter(np.arange(1, 6).astype('int'), train_loo, color='b', label='train')
pl.scatter(np.arange(1, 6).astype('int'), test_loo, color='k', label='test')
pl.legend()
pl.xticks(np.arange(1, 6))
pl.ylabel('LOO')
pl.xlabel('# parameters')
...
f, ax = pl.subplots(nrows=2, figsize=(6, 8), sharex=True)
ax[0].scatter(np.arange(1, 6), train_r2, color='b', label='train')
ax[0].scatter(np.arange(1, 6), test_r2, color='k', label='test')
ax[0].set_ylabel(r'$R^{2}$')
ax[0].legend()
ax[1].scatter(np.arange(1, 6), train_mae, color='b', label='train')
ax[1].scatter(np.arange(1, 6), test_mae, color='k', label='test')
